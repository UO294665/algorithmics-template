...
public class NumericSquareAll {
	...
	private void backtracking(int row, int col) {
		...
		if(!untouchables[row][col]) {
			for(int val = -5; val < 6; val ++) {
				board[row][col] = val;
				if(hasSolution() && containsPrevSolution()) {
					isSolved = true;
					solutions++;
					saveState();
				}
			...
			if(col == board.length-2 && row == board.length-2)
				return;
			if(col == board.length - 2) {
				if(isValidRow(row)) {
					backtracking(row + 1, 0);
	...
	private boolean containsPrevSolution() {
		if(solutionsReached.size()==0)
			return true;
		for(int[][]newBoard : solutionsReached) {
	...
	private boolean isValidDiagonal() {
		int result = 0;
		for(int i = 0; i<board.length-1;i++) {
			result +=board[i][i];
		}
